# BTC backtest engine E0-v6 exact component-contract plan

Stage: `E0-v6 design qualification`  
...
E0-v6 supersedes E0-v5 after its failed post-freeze review and supersedes only E0-v4's proposed
`next_E1` implementation path. E0-v4 and E0-v5 remain immutable incident and negative design
evidence; neither is retroactively treated as implementation authorization.
...
## Exact interface and component sequence

The machine interface specification freezes field name, scalar/container type, nullability,
producer and consumer for `RunContext`, `OrderIntention`, `ExecutionFact`, `TransitionPlan`,
`AccountState`, `EventRow` and `ComponentManifest`. Only C1 converts an authorized target into an
`OrderIntention` during its inherited phase clock. C2 or C3 converts that intention into an
`ExecutionFact`; C1 alone commits the fact to balances after verifying the plan's before-state,
run-context, source, rule, mandate, settings and scenario bindings are still current.
...
E0-v6 passes only when the machine contract and validator prove that:
...
- C0-C6 responsibilities and prohibited responsibilities do not overlap;
- E0-v1-v3 economics are inherited and E0-v4 is retained only as incident evidence;
- every row binds the complete run-context/implementation manifest; row-kind schemas separately
  require or forbid decision, availability, execution, funding-effective and accounting timestamps;
  event-specific digests are required or explicitly `not_applicable` by row kind;
...
The machine obligation registry cites exact JSON pointers in E0-v1-v3 and assigns each inherited
event-order, execution, funding, fee, margin, liquidation, gap, pair, control and lineage obligation
to one component and one unique probe. C6 explicitly requires accepted candle controls, the later
L2-control extension and the later pair-control extension.

Failure rejects E0-v6 before any component implementation. Passing permits only C0 under its new
...
- `research/btc/tests/test_unified_engine_e0_v6.py`; and
- a canonical pre-review bundle manifest binding all seven files before independent review.
