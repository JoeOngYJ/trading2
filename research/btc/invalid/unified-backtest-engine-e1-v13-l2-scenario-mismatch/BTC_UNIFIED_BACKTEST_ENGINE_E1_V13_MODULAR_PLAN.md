# BTC unified backtest engine E1-v13 modular architecture plan

Stage: `E1-v13 preimplementation`
Experiment ID: `btc-unified-backtest-engine-e1-v13`
Actionable arm: `no_trade`

## Scope

E1-v13 is offline, synthetic, zero-capital accounting infrastructure. It preserves the accepted
E0-v1–v4 economics and the V12 modular design while closing V12's two preflight defects: all four
...
snapshot freeze. Implementers may read E0-v1–v4, this V13 plan/contract, exact execution/mandate
authorities and the six V13 fixture authorities. They may not inspect, search, read, import,
...
The canonical files under `research/btc/fixtures/unified-engine-e1-v13/` are frozen inputs:
...
| semantic settings | `semantic-settings.json` | `cd23da773ade7b1b430802e741cf3db7dff3ce4252791ee1ea0b3199c9fcde2c` |
...
| four RunSpecs | `run-specs.json` | `fcca39232fecb58587ee096813243ebf89ab46b78de35a5cdf339334b6bd1066` |

Each RunSpec fixes its schema, ID, selected mandate ID/path/hash, exact 30-bps primary scenario,
80-bps severe scenario, permitted 40/80-bps sensitivity IDs, adapter, synthetic UTC partition
...
```text
src/trading_platform/btc_accounting_e1_v13/
...
```text
research/btc/tests/btc_accounting_e1_v13/
  test_canonical_authorities.py test_ledger.py test_pair.py
  test_controls_outputs.py test_integration.py
```

The candidate root `research/btc/candidates/unified-backtest-engine-e1-v13/` mirrors all eleven
...
Failure archives exact V13 bytes and requires a new ID; V13 is never patched in place.
