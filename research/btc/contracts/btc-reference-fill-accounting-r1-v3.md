...
Identity: `btc-reference-fill-accounting-r1-v3`. Frozen before implementation. Action `no_trade`.
...
Amounts are unsigned ordinary decimal strings with at most 32 digits total and at most 24
...
rounding source values. Use a fresh Decimal Context(prec=128, ROUND_HALF_EVEN). No rounding or
...
Successor to accepted R1-v2, which remains unchanged. Numeric domain only changes:32 total
digits,24 fractional places,10000 events. Inventory <1e36; absolute summed notionals and
equity <2e68; product scale <=48. At most117 significant digits; precision128 suffices.
Require exact Fraction mixed-scale and maximum-count tests; no rounding archived values.
