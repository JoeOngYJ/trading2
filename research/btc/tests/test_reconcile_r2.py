...
from research.btc.reconcile_r2 import SCENARIOS, LOW, HIGH, decode, gate, replay, close, checked_bytes
...
class R2Tests(unittest.TestCase):
    ...
    def test_gates(self):
        ...
        with self.assertRaises(ValueError):gate(r)
        r=fixture();r[0]['quantity']=D('0.000123456789012345')
        with patch('research.btc.reconcile_r2.account',side_effect=AssertionError('must not account')):
