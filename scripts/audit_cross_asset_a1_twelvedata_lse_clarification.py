#!/usr/bin/env python3
"""Audit the frozen exact-line LSE boundary clarification without strategy computation."""
...
DEFAULT_MANIFEST = REPO_ROOT / "artifacts/agent-level-experiment/cross-asset/a1-executable-universe-feasibility-v2/source-manifest.json"
V1_CONTRACT = REPO_ROOT / "config/experiments/cross-asset-a1-executable-universe-feasibility-v1.json"
V1_MANIFEST = REPO_ROOT / "artifacts/agent-level-experiment/cross-asset/a1-executable-universe-feasibility-v1/source-manifest.json"
...
def run(contract_path: Path, manifest_path: Path) -> tuple[Path, Path]:
    ...
    results, failures = audit_outcome(
        REPO_ROOT,
        contract,
        manifest_path,
        V1_CONTRACT,
    ...
    if report_path.exists() or evidence_path.exists():
        raise TwelveDataAuditError("write-once LSE clarification audit artifacts already exist")
    source_passed = not failures
    decision = (
        "exact_lse_source_clarification_passed"
        if source_passed
        else "exact_lse_source_clarification_rejected"
    )
    report = {
        ...
        "exact_lse_source_clarification_passed": source_passed,
        ...
        "reused_v1_evidence_sha256": contract["evidence_boundary"]["preceding_evidence"]["sha256"],
        "review_id": contract["review_id"],
        "sealed_2026_partition_accessed": False,
        "strategy_signals_generated": False,
    }
    report_path.write_text(canonical_json(report), encoding="utf-8")
    artifacts = [manifest_path, report_path, *sorted((output_root / "raw").glob("*.json"))]
    evidence = {
        "artifacts": [_artifact(path) for path in artifacts],
        "contract_sha256": sha256_file(contract_path),
        "decision": decision,
        "reused_artifacts": [
            contract["evidence_boundary"]["preceding_evidence"],
            contract["evidence_boundary"]["preceding_source_manifest"],
            contract["evidence_boundary"]["public_feasibility_facts"],
        ],
        "review_id": contract["review_id"],
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Audit the frozen LSE boundary clarification.")
