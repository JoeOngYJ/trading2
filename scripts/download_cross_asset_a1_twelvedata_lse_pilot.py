#!/usr/bin/env python3
"""Transactionally acquire the frozen exact-line Twelve Data LSE pilot."""
...
def run(contract_path: Path, timeout_seconds: float) -> Path:
    ...
    if output_root.exists():
        raise TwelveDataAuditError("write-once exact LSE artifact root already exists")
    output_root.parent.mkdir(parents=True, exist_ok=True)
    temp_root = Path(tempfile.mkdtemp(prefix=".twelvedata-lse-", dir=output_root.parent))
    ...
    try:
        for spec in specs:
            if spec.phase != current_phase:
                remaining = 61.0 - (time.monotonic() - phase_started)
                if remaining > 0:
            ...
            request = Request(
                ...
                headers={
                    ...
                    "User-Agent": "trading-research-exact-lse-qualification/1.0",
                },
                method="GET",
            )
            try:
                ...
                raise TwelveDataAuditError(
                    f"provider request failed for {spec.symbol} {spec.endpoint}: {message}"
                ) from exc
            if status != 200:
                raise TwelveDataAuditError(
            ...
            print(f"completed {spec.symbol} {spec.endpoint} phase {spec.phase}", flush=True)
        manifest = {
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the frozen exact-line Twelve Data LSE pilot.")
